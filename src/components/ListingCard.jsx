...
export function ListingCard({ listing }) {
  return (
    ...
    >
      ...
      <div className="p-2">
        <p className="text-sm font-semibold text-gray-900 truncate" title={listing.title}>{listing.title}</p>
        <p className="text-xs text-gray-600 mt-1">from ${listing.price_per_night} a day.</p>
      </div>
    </Link>
  );
}
